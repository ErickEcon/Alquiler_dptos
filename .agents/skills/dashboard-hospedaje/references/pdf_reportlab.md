# Informe PDF con ReportLab — Referencia

## src/pdf_report.py

```python
# pdf_report.py
# Genera un informe PDF profesional del negocio de hospedaje.
# Estructura: portada → resumen ejecutivo → tabla de ingresos → tabla de ocupación → gráficas
...
import pandas as pd
import matplotlib
matplotlib.use("Agg")   # Backend sin pantalla, necesario para generar imágenes en servidor
...
def generar_pdf(
    ...
    """
    Genera el informe PDF completo del negocio.
    Secciones: portada, KPIs, ingresos por habitación, ocupación, gráficas de barras.
    """
    ...
    # --- Tabla de ingresos anuales por habitación ---
    elementos += _seccion_ingresos_por_hab(estilos, metricas)
    elementos.append(Spacer(1, 0.8 * cm))

    # --- Tabla de tasa de ocupación ---
    elementos += _seccion_ocupacion(estilos, metricas)
    elementos.append(Spacer(1, 0.8 * cm))

    # --- Gráficas (convertidas a imagen PNG embebida) ---
...
def _seccion_portada(estilos: dict, m: MetricasNegocio) -> list:
    """Portada con nombre del negocio, fecha y métricas de alto nivel."""
    ...
    return [
        ...
        Spacer(1, 1.5 * cm),
        Paragraph(f"Período analizado: {min(m.años_disponibles)} – {max(m.años_disponibles)}", estilos["normal"]),
...
def _seccion_kpis(estilos: dict, m: MetricasNegocio) -> list:
    """Tabla de KPIs clave en formato visual destacado."""
    año_actual = max(m.años_disponibles) if m.años_disponibles else "N/A"
...
def _seccion_ingresos_por_hab(estilos: dict, m: MetricasNegocio) -> list:
    ...
    # Encabezado: habitación + un col por año + total
    ...
    ancho_col = 16 * cm / n_cols  # Distribuir el ancho disponible
...
def _seccion_ocupacion(estilos: dict, m: MetricasNegocio) -> list:
    """Tabla de tasa de ocupación por habitación."""
...
def _seccion_graficas(estilos: dict, m: MetricasNegocio) -> list:
    """Genera gráficas con matplotlib y las embebe como imágenes en el PDF."""
    elementos = [
...
def _grafica_ingresos_mensuales(m: MetricasNegocio) -> io.BytesIO | None:
    """Retorna la gráfica de ingresos mensuales como buffer PNG en memoria."""
...
def _grafica_ocupacion_mensual(m: MetricasNegocio) -> io.BytesIO | None:
    """Retorna la gráfica de tasa de ocupación mensual como buffer PNG."""
...
def _crear_estilos() -> dict:
    """Define los estilos tipográficos usados en el documento."""
...
def _tabla_con_estilo(
    ...
    """Crea una tabla ReportLab con estilos consistentes."""
